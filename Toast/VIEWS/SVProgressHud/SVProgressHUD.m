...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDBackgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.9];
        SVProgressHUDForegroundColor = [self colorWithHexString:@"CEAF5C"];
        SVProgressHUDCornerRadius = 14;
        if ([UIFont respondsToSelector:@selector(preferredFontForTextStyle:)]) {
            SVProgressHUDFont = [UIFont preferredFontForTextStyle:UIFontTextStyleSubheadline];
        } else {
            SVProgressHUDFont = [UIFont systemFontOfSize:15.0f];
...
//             if (windowOnMainScreen && windowIsVisible && windowLevelNormal) {
            if (windowOnMainScreen && windowIsVisible) {
...
- (CAShapeLayer *)createRingLayerWithCenter:(CGPoint)center radius:(CGFloat)radius lineWidth:(CGFloat)lineWidth color:(UIColor *)color {
    ...
    slice.frame = CGRectMake(center.x-radius, center.y-radius, radius*1.5, radius*1.5);
