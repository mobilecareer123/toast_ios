...
- (void)setUpUI {
    CGRect f = self.frame;
    CGFloat width = [UIApplication sharedApplication].keyWindow.bounds.size.width;
    CGFloat height = isIOS7? 61: f.size.height;
    self.frame = CGRectMake(f.origin.x, -height, width, height);
    
    CGRect cvF = self.containerView.frame;
    self.containerView.frame = CGRectMake(cvF.origin.x+10, cvF.origin.y, self.frame.size.width, cvF.size.height);
...
//        self.barStyle = UIBarStyleBlack;
    ...
    self.underlinelbl.layer.cornerRadius = 2.0;
    self.underlinelbl.clipsToBounds = YES;
    
    self.iConImage.layer.cornerRadius = 4.0;
